...
export function Home() {
  ...
  // Extract hero block from the CMS content[] array
  // Live schema: content: [{ type: "hero", data: { headline, sub_headline, background_image, stats[] } }]
  const heroBlock = extractContentBlock(pageData, 'hero', {});
  const hero = { ...heroBlock, ...(pageData?.hero ?? {}) };
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            ...
            >
              ...
              <h1 className="text-5xl md:text-7xl font-bold leading-tight mb-6">
                {(hero?.headline || hero?.title) ? (
                  <span dangerouslySetInnerHTML={{ __html: hero.headline || hero.title }} />
                ) : (
                  <>
                    Kebutuhan <br />
              ...
              <p className="text-lg text-muted-foreground mb-10 max-w-lg leading-relaxed">
                {hero?.sub_headline ?? hero?.subtitle ?? hero?.description ?? "Tingkatkan pengalaman kuliahmu bersama GIAT. Dari seragam akademik hingga merchandise eksklusif Tel-U, kami menyediakan semua yang kamu butuhkan."}
              </p>
            ...
            >
              <div className="relative z-10 rounded-[40px] overflow-hidden shadow-2xl border-8 border-white dark:border-slate-800 aspect-[4/5] md:aspect-square">
                <img 
                  src={hero?.background_image || hero?.image || "https://images.unsplash.com/photo-1523240795612-9a054b0db644?q=80&w=2070&auto=format&fit=crop"} 
