  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const allCats = [{ id: 'all', category_name: "Semua Produk" }, ...catsRes.data];
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="max-w-xl">
              ...
              <p className="text-muted-foreground">
                Produk berkualitas yang dikurasi untuk komunitas Telkom University. 
                Temukan kebutuhanmu dari pakaian akademik hingga minuman segar sehari-hari.
              </p>
            </div>
            <Button 
              variant="link" 
              className="text-giat-blue dark:text-blue-400 font-bold p-0 h-auto group"
              ...
              <TabsList className="bg-muted/50 p-1 rounded-full mb-12 h-auto flex-wrap justify-start">
                {categories.map((cat) => (
                  <TabsTrigger 
                    ...
                    className="rounded-full px-8 py-2.5 data-[state=active]:bg-giat-red data-[state=active]:text-white transition-all"
                  >
                    {cat.category_name}
                  </TabsTrigger>
                ))}
                ...
                <TabsContent key={cat.id} value={cat.id.toString()} className="mt-0">
                  {products.filter(p => cat.id === 'all' || p.category_id === cat.id).length === 0 ? (
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      {products
                        .filter((p) => cat.id === 'all' || p.category_id === cat.id)
                        .map((product, index) => (
                          <motion.div
                            key={product.id}
                            ...
                            <Card className="overflow-hidden border-none shadow-sm hover:shadow-xl transition-all duration-500 group rounded-[24px] bg-background h-full flex flex-col">
                              <div className="relative h-48 overflow-hidden bg-muted/20 flex items-center justify-center">
                                ...
                                {product.stock_status && product.stock_status !== "Tersedia" && (
                                  <div className="absolute top-4 left-4 bg-giat-blue text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full z-10 shadow-sm">
                              ...
                              <CardContent className="p-5 flex-grow flex flex-col justify-between">
                                <h3 className="text-lg font-bold mb-2 group-hover:text-giat-red transition-colors line-clamp-2">
                                  {product.name}
                                </h3>
