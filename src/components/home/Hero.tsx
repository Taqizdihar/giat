      ...
      <div className="container mx-auto px-6 py-12">
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            >
              {data?.badge ?? settings?.site_name ?? "Koperasi Mahasiswa Telkom University"}
            </motion.span>
            ...
            <p className="text-lg text-muted-foreground mb-10 max-w-lg leading-relaxed">
              {data?.subtitle ?? data?.description ?? "Tingkatkan pengalaman kuliahmu bersama GIAT. Dari seragam akademik hingga merchandise eksklusif Tel-U, kami menyediakan semua yang kamu butuhkan."}
            </p>
            <div className="flex flex-wrap gap-4">
              <Button 
                size="lg" 
                className="rounded-full px-8 bg-giat-red hover:bg-giat-red/90 text-white shadow-lg shadow-giat-red/20 group"
                render={<Link to={data?.cta_primary_link ?? "/katalog"} className="flex items-center justify-center" />}
              >
                {data?.cta_primary_text ?? "Lihat Katalog"}
              ...
              <Button 
                ...
                render={<a href={data?.cta_secondary_link ?? shopeeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center" />}
              >
                {data?.cta_secondary_text ?? "Kunjungi Shopee"}
            ...
            <div className="mt-12 flex items-center gap-8">
              <div className="flex -space-x-3">
                {(data?.avatars ?? [1, 2, 3, 4]).map((item: any, i: number) => (
              ...
              <p className="text-sm font-medium">
                <span className="text-giat-red font-bold">{data?.stats_count ?? "1000+"}</span> {data?.stats_label ?? "Mahasiswa terlayani semester ini"}
              </p>
            </div>
          </motion.div>
          ...
          >
            <div className="relative z-10 rounded-[40px] overflow-hidden shadow-2xl border-8 border-white dark:border-slate-800 aspect-[4/5] md:aspect-square">
              <img 
                src={data?.image ?? "https://images.unsplash.com/photo-1523240795612-9a054b0db644?q=80&w=2070&auto=format&fit=crop"} 
                alt={data?.image_alt ?? "Mahasiswa di Telkom University"} 
                className="w-full h-full object-cover"
                referrerPolicy="no-referrer"
                onError={(e) => { e.currentTarget.src = "https://placehold.co/600x400?text=Image+Unavailable"; }}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-8 text-white">
                <p className="text-sm font-medium opacity-80 mb-1">{data?.featured_label ?? "Produk Unggulan"}</p>
                <h3 className="text-2xl font-bold">{data?.featured_title ?? "Seragam Resmi"}</h3>
              </div>
            </div>
